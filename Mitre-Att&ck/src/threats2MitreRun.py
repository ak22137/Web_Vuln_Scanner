...
def creatReport(dataDict):
    ...
    fileExtention = "_Atk_%s.json" %str(dateTime)
...
def main():
    ...
    while True:
        ...
        if scenarioFile.lower() == 'exit' or scenarioFile.lower() == 'q':
            print('Exist...')
            break
        print("Select the AI-LLM data process program 1.MITRE-ATT&CK-Mapper")
        uInput = str(input())
        if uInput == '1':
...
#-----------------------------------------------------------------------------
def processThreatsScenario(scenarioFile):
    threatsMapper = threats2MitreMapper()
    threatsMapper.processScenarioFile(scenarioFile)
    print(f"Processed threats scenario file: {scenarioFile}")

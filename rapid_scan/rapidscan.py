...
def terminal_size():
    # Return a default terminal width
    return 80
...
tool_names = [
                ...
                #9
                ["theHarvester", "Scanning for emails using Google's passive search.","theHarvester",1],

                #10
                ["dnsrecon","Attempting Multiple Zone Transfers on Nameservers.","dnsrecon",1],
                ...
                #26
                ["lbd","Checking for DNS/HTTP Load Balancers.","lbd",1],
                ...
                #32
                ["dirb","Brute-forcing the target for Open Directories.","dirb",1],

                #33
                ["xsser","Checking for Cross-Site Scripting [XSS] Attacks.","xsser",1],
                ...
                #38
                ["dnsenum_zone_transfer","Attempting Zone Transfer.","dnsenum",1],
                ...
                #47
                ["uniscan_filebrute","Brute-forcing for Filenames on the Domain.","uniscan",1],

                #48
                ["uniscan_dirbrute", "Brute-forcing Directories on the Domain.","uniscan",1],

                #49
                # ["uniscan_ministresser", "Uniscan - Stress Tests the Domain.","uniscan",1],

                #50
                ["uniscan_rfi","Checking for LFI, RFI and RCE.","uniscan",1],
                ...
                #66
                ["dnsmap_brute","Brute-forcing Subdomains.","dnsmap",1],
                ...
                #78
                ["wapiti","Checking for SQLi, RCE, XSS and Other Vulnerabilities","wapiti",1],

                # #79
                # ["nmap_iis","Nmap - Checks for IIS WebDAV","nmap",1],

                #80
                ["whatweb","Checking for X-XSS Protection Header","whatweb",1],

                #81
                ["amass","Brute-forcing Domain for Subdomains","amass",1]
            ]
...
tools_precheck = [
    ["wapiti"], ["whatweb"], ["dirb"], ["uniscan"], ["lbd"], 
    ["dnsrecon"], ["xsser"], ["amass"], 
    ["dnsmap"], ["theHarvester"], ["dnsenum"]
]
...
def get_parser():
    ...
    parser.add_argument('--json-output', metavar='PATH', default='',
                        help='Write structured scan results to PATH.')
...
if args_namespace.help or (not args_namespace.update \
    ...
    while(tool < len(tool_names)):
        test_id = str(tool_names[tool][arg1])
        test_record = {"test_id": test_id, "name": str(tool_names[tool][arg2]),
                       "status": "running", "started_at": time.time(),
                       "completed_at": None, "duration_ms": None, "findings": [],
                       "errors": []}
        structured_tests[test_id] = test_record
    ...
    if args_namespace.json_output:
        structured_payload = {
            ...
            "scan_mode": "FULL",
            "target": target,
            "started_at": min((item["started_at"] for item in structured_tests.values()), default=time.time()),
            "completed_at": time.time(),
            "tests": list(structured_tests.values()),
            "errors": [error for test in structured_tests.values()
                        for error in test.get("errors", [])]
        }
        json_directory = os.path.dirname(args_namespace.json_output)
        if json_directory:
            os.makedirs(json_directory, exist_ok=True)
        with open(args_namespace.json_output, "w", encoding="utf-8") as json_file:
            json.dump(structured_payload, json_file, indent=2)
